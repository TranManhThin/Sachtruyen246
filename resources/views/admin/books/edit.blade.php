...
@section('page-content')
    <form action="{{ route('book.update',['book'=>$bookId->id]) }}" name="formPost" id="formPost" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        ...
        <div class="mb-3">
            ...
            <input onkeyup="changeToSlug()" type="text" class="form-control @error('name') is-invalid @enderror"
                id="name" name="name" aria-describedby="categoryHelp" value="{{ $bookId->name }}">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('author') is-invalid @enderror" id="author" name="author"
                aria-describedby="categoryHelp" value="{{ $bookId->author }}">
            @error('author')
                <p class="invalid-feedback d-block">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="desc" id="desc" rows="4" class="form-control @error('desc') is-invalid @enderror"
                style="resize: none">{{ $bookId->description }}</textarea>
            <p></p>
        </div>
        ...
        <div class="mb-3">
            ...
            @enderror
            <br>
            <img src="{{ $bookId->image }}" alt="" class="rounded" style="box-shadow: 0 0 10px rgba(0, 0, 0, 0.5); width: 500px; height: 550px;">
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Nội dung</label>
            <textarea name="content" id="summernote" class="form-group @error('content') is-invalid @enderror">{{ $bookId->content }}</textarea>
            @error('content')
                <p class="invalid-feedback d-block">{{ $message }}</p>
            @enderror
        </div>
